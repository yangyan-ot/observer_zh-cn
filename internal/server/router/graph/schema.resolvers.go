...
// GetSoftwareVersion is the resolver for the getSoftwareVersion field.
func (r *queryResolver) GetSoftwareVersion(ctx context.Context) (string, error) {
	binaryVersion := r.CurrentVersion.String()
	buildCommit := r.CurrentBuild.GetCommit()
	buildTime := r.CurrentBuild.GetTime()
	...
	return fmt.Sprintf("%s-%s%s", binaryVersion, buildCommit, buildTimeStr), nil
}
